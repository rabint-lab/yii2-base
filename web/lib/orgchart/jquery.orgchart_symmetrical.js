    ...
    function OrgChart($container, opts) {
        ...
        this.getInputElement = function(id) {
            ...
            if (opts.typePrompt != false) {
                selected = (nodes[id].data.type == opts.typePrompt) ? 'selected' : ''
                $s2 += '<option ' + selected + ' value="' + opts.typePrompValue + '">' + opts.typePrompt + '</option>';
            }
            for (type in opts.types) {
                selected = (nodes[id].data.type == type) ? 'selected' : ''
                $s2 += '<option ' + selected + ' value="' + type + '">' + opts.types[type] + '</option>';
            }
        ...
        this.getViewElement = function(id) {
            ...
            if (typeof data.type !== 'undefined') {
                i = data.type;
                $desc = (typeof opts.types[i] !== 'undefined') ? opts.types[i] : opts.typePrompt;
                $desc = ($desc == opts.typePrompValue) ? opts.typePrompt : $desc;
                typeString = '<p>' + $desc + '</p>';
            }
        ...
        this.startEdit = function(id) {
            ...
            // inputElement.on('click', '.saveNode', function(event) {

            // })
        }
        this.nodeLevels = function($structure,$nodeID)
        {
            if ($nodeID == null) {
                for ($node in $structure) {
                    $node['level'] =  self.nodeLevels($structure, $structure[$node]['id']);
                }
                return $structure;
            } else {
                $node = $structure[$nodeID];
                if ($node['parent']==0) {
                    return 1;
                }
                return self.nodeLevels($structure, $node['parent']) + 1;
            }
        }
        ...
        this.newNode = function(parentId) {
            ...
            var nodeLevels=[];
            for (anode in nodes) {
                nodeLevels[anode.id] = anode.parent;
            }
