...
function ModalRemote(modalId) {
    ...
    function errorRemoteResponse(response) {
        if(response.status==301 || response.status==302){
            ...
            this.setTitle('بروزرسانی ');
            this.setContent('در حال بروزرسانی صفحه');
    ...
    function successRemoteResponse(response) {
        ...
        if(respIsHtml){
            //console.log('resp is html3');
            let html = $.parseHTML(response);
            let titleDiv = $(html).find('.ajaxModalTitle');
