...
class security {
    ...
    public static function xss_clean($str, $is_image = FALSE) {
        ...
        if (stripos($str, '%') !== false) {
            do {
                ...
                $str = preg_replace_callback('#%(?:\s*[0-9a-f]){2,}#i', array(self, '_urldecodespaces'), $str);
            } while ($oldstr !== $str);
            unset($oldstr);
        }
        ...
        do {
            ...
            if (preg_match('/<a/i', $str)) {
                $str = preg_replace_callback('#<a(?:rea)?[^a-z0-9>]+([^>]*?)(?:>|$)#si', array(self, '_js_link_removal'), $str);
            }
    ...
    public static function xss_hash() {
        if (self::$_xss_hash === NULL) {
            ...
            self::$_xss_hash =($rand === FALSE) ? md5(uniqid(mt_rand(), TRUE)) : bin2hex($rand);
        }
     ...
     * @return	string
     */
    public static function _urldecodespaces($matches) {
        $input = $matches[0];
     ...
     * @return	string
     */
    public static function _compact_exploded_words($matches) {
     ...
     * @return	string
     */
    public static function _sanitize_naughty_html($matches) {
     ...
     * @return	string
     */
    public static function _js_link_removal($match) {
        return str_replace(
                $match[1], preg_replace(
                        '#href=.*?(?:(?:alert|prompt|confirm)(?:\(|&\#40;)|javascript:|livescript:|mocha:|charset=|window\.|document\.|\.cookie|<script|<xss|d\s*a\s*t\s*a\s*:)#si', '', self::_filter_attributes($match[1])
                ), $match[0]
        );
    }
     ...
     * @return	string
     */
    public static function _js_img_removal($match) {
        return str_replace(
                $match[1], preg_replace(
                        '#src=.*?(?:(?:alert|prompt|confirm|eval)(?:\(|&\#40;)|javascript:|livescript:|mocha:|charset=|window\.|document\.|\.cookie|<script|<xss|base64\s*,)#si', '', self::_filter_attributes($match[1])
                ), $match[0]
        );
    }
     ...
     * @return	string
     */
    public static function _convert_attribute($match) {
     ...
     * @return	string
     */
    public static function _filter_attributes($str) {
        $out = '';
     ...
     * @return	string
     */
    public static function _decode_entity($match) {
        ...
        return str_replace(
                self::xss_hash(), '&', self::entity_decode($match, self::charset)
        );
    }
     ...
     * @return 	string
     */
    public static function _do_never_allowed($str) {
...
//                $output[$key] = self::arrayXssClean($array, $key);
//            }
//
//            return $output;
//        }
...
//        }
//
//        return \yii\helpers\HtmlPurifier::process($data);
//        return self::xss_clean($value);
    }

    /**
     * 
     * @param \yii\web\UploadedFile $file
     * @return boolean is Allowed file
     */
    public static function checkAllowedUploadedFile($file) {

        $fileMime = strtolower($file->type);
        $fileExt = strtolower(\rabint\helpers\file::getFileExt($file->name));
     ...
     * @return boolean is Allowed file
     */
    public static function checkAllowedMimeAndExt($mime,$ext) {
        $fileMime = strtolower($mime);
    ...
    public static function checkAllowedType($mime, $moreWhiteType = "", $moreBlackType = "") {
         ...
         * create white list
         */
        $whiteList = strtolower(config("SECURITY.whiteListMime", ""));
        if (!empty($whiteList)) {
...
//            die("not in white type");
         ...
         */
        $blackListArray = [];
        $blackList = strtolower(config("SECURITY.blackListMime", ""));
        if (!empty($blackList)) {
...
//        die("type is okey");
    ...
    public static function checkAllowedExt($ext, $moreWhiteExt = "", $moreBlackExt = "") {
        $ext = strtolower($ext);
        /**
         * check white list ####################################################
         */
         ...
         * create white list
         */
        $whiteList = strtolower(config("SECURITY.whiteListExt", ""));
        if (!empty($whiteList)) {
            $listArray = explode(",", $whiteList);
            $listArray = array_map("trim", $listArray);
            $whiteListArray = $listArray;
...
//            die("not in white Ext");
         ...
         */
        $blackListArray = [];
        $blackList = strtolower(config("SECURITY.blackListExt", ""));
        if (!empty($blackList)) {
            $listArray = explode(",", $blackList);
            $listArray = array_map("trim", $listArray);
            $blackListArray = $listArray;
