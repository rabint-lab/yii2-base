...
class NestedSetsBehavior extends Behavior {
    ...
    public function moveNodeTo($newParent) {
        $old = $this->owner->findOne($this->owner->id);
        $oldParent = $this->owner->parents(1)->one();

        /* =================================================================== */

        $nodeSize = $old->rgt - $old->lft;
        $newLft = $newParent->rgt;
        $newRgt = $newParent->rgt + $nodeSize;
        $newRoot = $newParent->root;
        $newLevel = $newParent->level + 1;

        //change all newParent broder AND root ================================
        $this->owner->updateAll(
                [
            'lft' => new Expression('lft+' . ($nodeSize + 1)),
                ], "root = {$newParent->root} AND " .
                "lft > {$newParent->rgt} "
        );
        $this->owner->updateAll(
                [
            'rgt' => new Expression('rgt+' . ($nodeSize + 1)),
                ], "root = {$newParent->root} AND " .
                "rgt >= {$newParent->rgt} "
        );
        /* change all node childs ================================ */

        if ($newLevel > $old->level) {
            $levelPar = 'level+' . ($newLevel - $old->level);
        } else {
            $levelPar = 'level-' . ($old->level - $newLevel);
        }
        if ($newLft > $old->lft) {
            $lftPar = 'lft+' . ($newLft - $old->lft);
        } else {
            $lftPar = 'lft-' . ($old->lft - $newLft);
        }
        if ($newRgt > $old->rgt) {
            $rgtPar = 'rgt+' . ($newRgt - $old->rgt);
        } else {
            $rgtPar = 'rgt-' . ($old->rgt - $newRgt);
        }
        $res = $this->owner->updateAll(
                [
            'root' => $newRoot,
            'level' => new Expression($levelPar),
            'lft' => new Expression($lftPar),
            'rgt' => new Expression($rgtPar),
                ], "root = {$old->root} AND " .
                "level >= {$old->level} AND " .
                "lft >= {$old->lft} AND " .
                "rgt <= {$old->rgt}  "
        ...
        //change all OLDParent broder AND root ================================
        if (!empty($oldParent)) {
            $this->owner->updateAll(
                    [
                'lft' => new Expression('lft-' . ($nodeSize + 1)),
                    ], "root = {$oldParent->root} AND " .
                    "lft > {$old->lft} "
            );
            $this->owner->updateAll(
                    [
        ...
        $this->owner->level = $newLevel;
        $this->owner->lft = $newLft;
        $this->owner->rgt = $newRgt;
        $this->owner->root = $newRoot;

        return $this->owner->save();
    }

    //
    public function nestedDelete() {
//      $this->owner->removeImage($model->image);
//      $this->owner->delete();
        return $this->owner->deleteWithChildren();
    }

    public function nestedSave() {
        if (!$this->owner->isNewRecord AND $this->owner->root == $this->owner->id) {
            return $this->owner->save();
        } elseif ($this->owner->root != $this->owner->id) {
            $ParentModel = $this->owner->findOne($this->owner->root);
            //$this->owner->alias = (!empty($this->owner->alias)) ? $this->owner->alias : $ParentModel->alias . ' ' . $this->owner->title;
            if ($this->owner->isNewRecord) {
                $this->makeRoot();
            }
            return $this->moveNodeTo($ParentModel);
        } else {
//           $this->owner->alias = empty($this->owner->alias) ? $this->owner->title : $this->owner->alias;
//            $this->owner->root = 0;
//            $this->owner->level = 0;
