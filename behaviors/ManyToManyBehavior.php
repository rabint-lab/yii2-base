...
class ManyToManyBehavior extends Behavior {
    ...
    /**
     * @param \common\models\base\ActiveQuery $relation
        ...
        try {
            ...
            $connection->createCommand()
                    ->delete($junctionTable, ArrayHelper::merge(
                                    [$junctionColumn => $primaryModelPk], $this->getCustomDeleteCondition($attributeName)
                    ))
                    ->execute();
            ...
            if (!empty($bindingKeys)) {
                ...
                foreach ($bindingKeys as $relatedPk) {
                    ...
                    $sourceTableParams = $this->getSourceTableParams($attributeName);
                    if (empty($sourceTableParams)) {
                        continue;
                    }
                    if (!is_numeric($relatedPk)) {
                        $relationModelClass = $relation->modelClass;
                        $has = $relationModelClass::findOne([$sourceTableParams => $relatedPk]);
                    } else {
                        $has = $relationModelClass::findOne($relatedPk);
                    }
                    if ($has === NULL) {
                    ...
                    foreach ($junctionRows as $jr) {
                        if (isset($jr[1]) and $jr[1] == $row[1] OR $row[1] == NULL) {
                            $repetitive = TRUE;
                        }
                    }
                    if (!$repetitive) {
...
//                }
    ...
    private function saveOneToManyRelation($relation, $attributeName) {
        ...
        try {
            ...
            $connection->createCommand()
                    ->update(
                            $manyTable, [$manyTableFkColumn => $defaultValue], [$manyTableFkColumn => $manyTableFkValue])
                    ->execute();
            ...
            if (!empty($bindingKeys)) {
                $connection->createCommand()
                        ->update(
                                $manyTable, [$manyTableFkColumn => $manyTableFkValue], ['in', $manyTablePkColumn, $bindingKeys])
                        ->execute();
            }
            $transaction->commit();
        } catch (Exception $ex) {
